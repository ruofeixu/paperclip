  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("costs:financeLedger.title")}</CardTitle>
        <CardDescription>
          {t("costs:financeLedger.description")}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("costs:financeLedger.debits")}
          value={formatCents(debitCents)}
          subtitle={t("costs:financeLedger.debitsSubtitle", { count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("costs:financeLedger.credits")}
          value={formatCents(creditCents)}
          subtitle={t("costs:financeLedger.creditsSubtitle")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("costs:financeLedger.net")}
          value={formatCents(netCents)}
          subtitle={t("costs:financeLedger.netSubtitle")}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("costs:financeLedger.estimated")}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("costs:financeLedger.estimatedSubtitle")}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("sidebar:costs") }]);
  }, [setBreadcrumbs, t]);
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costs:providers.all")}</span>
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={t("costs:noCompany")} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{t("costs:header.title")}</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("costs:header.description")}
                </p>
            </div>
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{t("costs:dateRange.to")}</span>
              <input
                type="date"
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              ...
              subtitle={t("costs:metrics.inferenceSpendSubtitle", { tokens: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ...
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("costs:metrics.budgetUsed", { spent: formatCents(spendData.summary.spendCents), total: formatCents(spendData.summary.budgetCents) })
                    : t("costs:metrics.budgetNoCap")
              }
              icon={Coins}
            />
            <MetricTile
              ...
              subtitle={t("costs:metrics.financeNetSubtitle", { debits: formatCents(financeData?.summary.debitCents ?? 0), credits: formatCents(financeData?.summary.creditCents ?? 0) })}
              icon={ReceiptText}
            />
            <MetricTile
              label={t("costs:metrics.financeEvents")}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("costs:metrics.financeEventsSubtitle", { estimated: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costs:dateRange.customPrompt")}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs:inferenceLedger.title")}</CardTitle>
                    <CardDescription>
                      {t("costs:inferenceLedger.description")}
                    </CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("costs:inferenceLedger.budgetLabel", { amount: formatCents(spendData.summary.budgetCents) })
                            : t("costs:inferenceLedger.unlimitedBudget")}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("costs:inferenceLedger.usage")}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("costs:inferenceLedger.utilization", { percent: spendData.summary.utilizationPercent })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs:byAgent.title")}</CardTitle>
                    <CardDescription>{t("costs:byAgent.description")}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("costs:byAgent.empty")}</p>
                    ) : (
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("costs:byAgent.in")} {formatTokens(row.inputTokens + row.cachedInputTokens)} · {t("costs:byAgent.out")} {formatTokens(row.outputTokens)}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {row.apiRunCount > 0 ? t("costs:byAgent.apiRuns", { count: row.apiRunCount }) : t("costs:byAgent.apiRuns", { count: 0 })}
                                    {" · "}
                                    {row.subscriptionRunCount > 0
                                      ? t("costs:byAgent.subscriptionRuns", { count: row.subscriptionRunCount })
                                      : t("costs:byAgent.subscriptionRuns", { count: 0 })}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs:byProject.title")}</CardTitle>
                      <CardDescription>{t("costs:byProject.description")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs:byProject.empty")}</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? t("costs:byProject.unattributed")}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costs:finance.timelineEmpty")} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("costs:budgetControl.title")}</CardTitle>
                  <CardDescription>
                    {t("costs:budgetControl.description")}
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("costs:metrics.activeIncidents")}
                    value={String(activeBudgetIncidents.length)}
                    subtitle={t("costs:metrics.activeIncidentsSubtitle")}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("costs:metrics.pendingApprovals")}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("costs:metrics.pendingApprovalsSubtitle")}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("costs:metrics.pausedAgents")}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("costs:metrics.pausedAgentsSubtitle")}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("costs:metrics.pausedProjects")}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("costs:metrics.pausedProjectsSubtitle")}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("costs:budgetControl.activeIncidentsTitle")}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("costs:budgetControl.activeIncidentsDescription")}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                    ...
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold">
                          {scopeType === "company" ? t("costs:budgetControl.companyBudgets") :
                           scopeType === "agent" ? t("costs:budgetControl.agentBudgets") :
                           t("costs:budgetControl.projectBudgets")}
                        </h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costs:budgetControl.companyBudgetsDescription")
                            : scopeType === "agent"
                              ? t("costs:budgetControl.agentBudgetsDescription")
                              : t("costs:budgetControl.projectBudgetsDescription")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      {t("costs:budgetControl.noPolicies")}
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs:providers.empty")}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs:billers.empty")}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs:finance.byBiller")}</CardTitle>
                      <CardDescription>{t("costs:finance.byBillerDescription")}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs:finance.empty")}</p>
                      ) : (

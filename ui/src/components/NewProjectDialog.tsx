...
export function NewProjectDialog() {
  ...
  async function handleSubmit() {
    ...
    if (localRequired && !isAbsolutePath(localPath)) {
      setWorkspaceError(t("dialogs:newProject.error.absolutePath"));
      return;
    }
    if (repoRequired && !isGitHubRepoUrl(repoUrl)) {
      setWorkspaceError(t("dialogs:newProject.error.invalidRepo"));
      return;
    }
      ...
      >
        ...
        <div className="px-4 pb-2">
          <MarkdownEditor
            ...
            placeholder={t("dialogs:newProject.description")}
            bordered={false}
        ...
        <div className="px-4 pb-3 space-y-3 border-t border-border">
          <div className="pt-3">
            <p className="text-sm font-medium">{t("dialogs:newProject.workspaceSetup")}</p>
            <p className="text-xs text-muted-foreground">{t("dialogs:newProject.workspaceSetupDesc")}</p>
          </div>
          <div className="grid gap-2 sm:grid-cols-3">
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">{t("dialogs:newProject.localFolderDesc")}</p>
            </button>
            <button
              type="button"
              className={cn(
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                {t("dialogs:newProject.repo")}
              </div>
              <p className="mt-1 text-xs text-muted-foreground">{t("dialogs:newProject.repoDesc")}</p>
            </button>
            <button
              type="button"
              className={cn(
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">{t("dialogs:newProject.bothDesc")}</p>
            </button>
          </div>

          {(workspaceSetup === "local" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">{t("dialogs:newProject.localPath")}</label>
              <div className="flex items-center gap-2">
                <input
                  className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                  value={workspaceLocalPath}
                  onChange={(e) => setWorkspaceLocalPath(e.target.value)}
                  placeholder={t("dialogs:newProject.localPathPlaceholder")}
                />
                <ChoosePathButton />
              </div>
            </div>
          ...
          {(workspaceSetup === "repo" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">{t("dialogs:newProject.repoUrl")}</label>
              <input
                className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs outline-none"
                value={workspaceRepoUrl}
                onChange={(e) => setWorkspaceRepoUrl(e.target.value)}
                placeholder={t("dialogs:newProject.repoPlaceholder")}
              />
            </div>
          )}
          {workspaceError && (
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{t("dialogs:newProject.error.createFailed")}</p>
          ) : (
            <span />
          )}
          <Button
          ...
          >
            {createProject.isPending ? t("dialogs:newProject.creating") : t("dialogs:newProject.create")}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
